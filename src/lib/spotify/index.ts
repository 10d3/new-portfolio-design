...
async function getUserAccessToken() {
  if (!USER_ACCESS_TOKEN) {
    if (!USER_REFRESH_TOKEN) {
      throw new Error('No access token or refresh token. User needs to authorize first.');
    }
...
export function getAuthorizationUrl() {
  ...
  const redirectUri = process.env.SPOTIFY_REDIRECT_URI || 'http://localhost:3000/callback'; // Must match what's registered in Spotify app
...
export async function exchangeCodeForTokens(authorizationCode: string, redirectUri: string) {
  const basic = Buffer.from(`${CLIENT_ID}:${CLIENT_SECRET}`).toString('base64');
