...
export interface WakatimeStats {
  ...
  totalHours: number; // Note: Only last 7 days for free accounts
  projects?: WakatimeProject[];
  devTools?: WakatimeDevTools;
  dataRange?: string; // Indicates the time range of the data
}
...
function parseTokensFromEnv() {
  ...
  // Debug logging (be careful not to log actual tokens in production)
...
const wakaStat = async () => {
  try {
    ...
    if (isTokenExpired(expiresAt)) {
      ...
      try {
        const tokenData = await refreshWakatimeAccessToken(refreshToken);
    ...
    // Fetch both stats and projects with current (possibly refreshed) token
    const [stats, summaries] = await Promise.all([
      fetchWakatimeStats(currentAccessToken),
      fetchWakatimeSummaries(currentAccessToken),
    ]);
...
export async function exchangeWakatimeCodeForToken(code: string) {
  ...
  if (!clientId || !clientSecret || !redirectUri)
    throw new Error("Missing WakaTime env vars");
...
export async function fetchWakatimeStats(accessToken: string) {
  ...
  if (!response.ok) {
    ...
    console.error("WakaTime stats API error:", response.status, errorText);
...
export async function fetchWakatimeSummaries(
  ...
  console.log(`Fetching WakaTime summaries for range: ${range}...`);
  const response = await fetch(
    `https://wakatime.com/api/v1/users/current/summaries?range=${range}`,
    {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
  ...
  if (!response.ok) {
    const errorText = await response.text();
    console.error("WakaTime summaries API error:", response.status, errorText);
    throw new Error(`WakaTime API error: ${response.status} - ${errorText}`);
  }
  const data = await response.json();
...
export function parseWakatimeStats(
  ...
  const languages: WakatimeLanguage[] = (stats?.data?.languages || []).map(
    (lang: any) => ({
      name: lang.name,
...
export function parseWakatimeProjects(summaries: any): WakatimeProject[] {
  ...
  return projects
    .map((project: any) => {
      ...
      const percent =
        totalSeconds > 0 ? (project.total_seconds / totalSeconds) * 100 : 0;
...
export async function fetchWakatimeStatsWithAutoRefresh(
  accessToken?: string,
  refreshToken?: string
) {
...
export async function fetchWakatimeSummariesWithAutoRefresh(
  accessToken?: string,
  refreshToken?: string,
  range: string = "last_7_days"
) {
